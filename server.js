...
app.get('/recount-cart/:id', function(req,res,next){
	var id = req.params.id;
	var recountedCart = new Cart(req.session.cart);
	recountedCart.deleteItem(id);
	req.session.cart = recountedCart;
	res.send(recountedCart);
})

app.post('/save-cart', function(req,res,next){
	var recountCart = new Cart(req.session.cart);
	recountCart.recount(req.body);
	req.session.cart = recountCart;
	res.send(recountCart);
});
